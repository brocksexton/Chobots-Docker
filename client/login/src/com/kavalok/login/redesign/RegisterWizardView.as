package com.kavalok.login.redesign
{
	...
	import com.kavalok.login.redesign.registration.BehaviorView;
	import com.kavalok.login.redesign.registration.ColorAgeView;
	...
	public class RegisterWizardView extends ViewStackPage
	{
		...
		private var _behaviorView : BehaviorView;
		private var _colorAgeView : ColorAgeView;
		...
		public function RegisterWizardView(content:McRegisterView)
		{
			_content = content;
			super(content);
			...
			_behaviorView = new BehaviorView(_content.viewStack.behaviorPage);
			_behaviorView.nextEvent.addListener(onNext);
			_behaviorView.backEvent.addListener(onBehaviorBack);
			_behaviorView.data = _registrationData;
			
			_colorAgeView = new ColorAgeView(_content.viewStack.colorPage);
			_colorAgeView.nextEvent.addListener(onNext);
			_colorAgeView.backEvent.addListener(onBack);
			_colorAgeView.data = _registrationData;
			_colorAgeView.content.visible=false;
			...
			_viewStack.addPage(_behaviorView);
			//_viewStack.addPage(_colorAgeView);
			_viewStack.addPage(_registrationView);
			
			_viewStack.selectedPage = _behaviorView;
