	...
	public class LocationChar implements IBubbleSource
	{
		...
		public function set showName(value : Boolean) : void
		{
			if(_showName != value)
			{
				...
				if(value)
				{
					_nameTip = new ToolTipText(CHAR_NAME_SIZE);
					_nameTip.margin = 1;
					_nameTip.text = _char.id;
				...
				{
					ToolTips.registerObject(_boundSprite, _char.id);
		...
		public function get bubblePosition():Point
		{
			...
			return (_model.isReady)
				? new Point(modelPosition.x, _model.getBounds(_content).top + 2)
				: new Point(modelPosition.x, _boundSprite.getBounds(_content).top + 2);
		}
		...
		public function moveByPath(path:Array):void
		{
			if (teleportMode && path.length > 0)
			{
				...
				var newDirection:int = Directions.getDirection(
				...
				TweenLite.to(_content, 1.0, 
					{
						...
						onComplete:_moveCompleteEvent.sendEvent
					}
				);
		...
		{
			var directionNum:int = (moonwalkMode)
				? Directions.invertDirections[newDirection]
				: newDirection;
		...
		{
			return (isStaing || isIdle)
				&& _char.tool == null
				&& _content.parent
				&& _content.parent.name == LocationBase.Z_SPRITE_NAME;
		}
		...
		{
			var bitmapEnabled:Boolean = 
				(_model.modelName == CharModels.STAY || _animationCompleted) && !_flyMode
				|| !_model.modelAnimation;
		...
		public function get shadow():CharModel { return _shadow;	}
		...
		public function get flyMode():Boolean { return _flyMode; }
		
		public function get clickEvent():EventSender { return _clickEvent; }
		public function get moveCompleteEvent():EventSender { return _moveCompleteEvent; }
		public function get moveStartEvent():EventSender { return _moveStartEvent; }
	}
}