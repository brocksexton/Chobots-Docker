package com.kavalok.messenger
{
	...
	public class Messenger extends Window
	{
		...
		private function initButtons():void
		{
			...
			ToolTips.registerObject(_content.inviteButton, 'inviteFriendTitle', ResourceBundles.KAVALOK);
		...
		private function refresh(sender:Object = null):void
		{
			...
			_selection = Arrays.getByRequirement(
				items, new CheckedRequirement());
			
			var checkedExists:Boolean = (_selection.length > 0);
			
			var unCheckedExists:Boolean = Arrays.containsByRequirement(
				items, new UnCheckedRequirement());
				
			var onlineExists:Boolean = Arrays.containsByRequirement(
				items, new OnlineRequirement());
				
			var offlineItems:Array = Arrays.getByRequirement(
				items, new OfflineRequirement());
				
			GraphUtils.setBtnEnabled(_content.teleportButton, onlineExists);
			GraphUtils.setBtnEnabled(_content.messageButton, checkedExists
				&& Global.notifications.chatEnabled && offlineItems.length < 8);
		...
		private function createListGroup(
			items:Array, headerClass:Class, itemClass:Class, caption:String):void
		{
			if (items.length == 0)
				return;
			...
			for each (var friend:FriendTO in items)
			{
				...
				item.checked = Arrays.containsByRequirement(_selection,
					new PropertyCompareRequirement('name', friend.login));
