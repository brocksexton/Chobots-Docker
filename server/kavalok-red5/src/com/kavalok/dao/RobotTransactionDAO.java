...
public class RobotTransactionDAO extends DAO<RobotTransaction> {
  ...
  public List<Object[]> findForDates(Long partnerId, Date minDate, Date maxDate) {
    if (partnerId != null) {
      Query query =
          getSession()
              .getNamedQuery(QueriesNames.ROBOT_TRANSACTION_STATISTICS_SELECT_PROCESSED_PARTNER);
      ...
      Query query =
          getSession().getNamedQuery(QueriesNames.ROBOT_TRANSACTION_STATISTICS_SELECT_PROCESSED);
      query.setParameter("minDate", minDate);
      query.setParameter("maxDate", maxDate);
      return query.list();
    }
