...
public class CompetitionResultDAO extends DAO<CompetitionResult> {
  ...
  public Long sizeOfCompetitionResults(Competition competition) {
  ...
  public Object[] getCompetitionResult(Competition competition, User user) {
    Criteria criteria =
        getCompetitionResultsCriteria(competition, getCompetitionResultProjection(), true);
    criteria.add(Restrictions.eq("user", user));
    return (Object[]) criteria.uniqueResult();
  }

  public List<?> getCompetitionResults(
      Competition competition, Integer firstResult, Integer maxResults) {
    Criteria criteria =
        getCompetitionResultsCriteria(competition, getCompetitionResultProjection(), true);
  ...
  private Criteria getCompetitionResultsCriteria(
      Competition competition, Projection projection, boolean addOrderByScore) {
    Criteria criteria = createCriteria();
    criteria.add(Restrictions.eq("competition", competition));
    if (addOrderByScore) criteria.addOrder(Order.desc("score"));
  ...
  public Long countByCompetitors(User user, User competitor, Competition competition) {
    Criteria criteria = createCriteria();
    criteria.add(Restrictions.eq("user", user));
