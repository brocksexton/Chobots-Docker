...
public class UserDAO extends LoginDAOBase<User> {
  ...
  public Integer getActivations(Date minDate, Date maxDate) {
    Criteria criteria = createCriteria(minDate, maxDate);
    criteria.add(
        Restrictions.or(Restrictions.isNull("activationKey"), Restrictions.eq("activated", true)));
    return (Integer) criteria.uniqueResult();
  }
  ...
  public List<User> findByPartner(
      Partner partner, Date from, Date to, Integer firstResult, Integer maxResults) {
    Criteria criteria = createCriteria();
    addCreatedRectriction(from, to, criteria);
  ...
  private Criteria createReferrerCriteria(
      Partner referrer, Integer firstResult, Integer maxResults) {
    Criteria criteria = createReferrerCriteria(referrer);
    criteria.setFirstResult(firstResult);
    criteria.setMaxResults(maxResults);
    return criteria;
  ...
  public void removeFriendship(Long userId, Long friendId) throws HibernateException, SQLException {
    String sqlQuery =
        "delete from User_User where (User_id=%1$d and friends_id=%2$d) "
