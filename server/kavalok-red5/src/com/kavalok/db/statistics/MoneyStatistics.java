...
@NamedQueries({
  @NamedQuery(
      name = QueriesNames.MONEY_STATISTICS_SELECT_COUNT,
      query =
          "select count(*) from MoneyStatistics as item "
              + "where item.date < :maxDate and item.date > :minDate "
              + "group by item.user"),
  ...
  @NamedQuery(
      name = QueriesNames.MONEY_STATISTICS_SELECT_FOR_USERS,
      query =
          "select item.user, sum(item.money) "
              + "from MoneyStatistics as item "
              + "where item.date < :maxDate and item.date > :minDate "
              + "group by item.user "
              + "order by sum(item.money) desc")
})
public class MoneyStatistics implements Serializable {
