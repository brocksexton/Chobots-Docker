...
public class PetService extends DataServiceBase {
  ...
  private GameChar getGameChar() {
    return UserManager.getInstance().getCurrentUser().getChar(getSession());
  }

  public void saveParams(Integer health, Integer food, Integer loyality, Boolean atHome) {
  ...
  public PetTO savePet(PetTO petTO) {
    ...
    if (result.getSafety() != MessageSafety.SAFE) return null;
