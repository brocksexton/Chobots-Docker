...
public class StatisticsService extends DataServiceNotTransactionBase {
  ...
  public PagedResult<MoneyEarnedTO> getMoneyEarned(
      Date minDate, Date maxDate, Integer firstResult, Integer maxResults) {
  ...
  public List<MembersAgeTO> getMembersAge(Date from, Date to, Integer period) {
    ...
    return ReflectUtil.convertBeansByConstructor(
        historyDAO.countByUserAge(from, to, period), MembersAgeTO.class);
  }
  ...
  public List<ActivationStatisticsTO> getActivationChart(
      Date minDate, Date maxDate, Integer count) {
  ...
  public PagedResult<UserLoginStatisticsTO> getUserLogins(
      Date minDate, Date maxDate, Integer firstResult, Integer maxResults) {
  ...
  public List<TransactionStatisticsTO> getTransactionStatistics(String minDate, String maxDate)
    ...
    for (Object[] objects :
        new TransactionDAO(getSession()).findForDatesOld(partnerId, minD, maxD)) {
  ...
  @SuppressWarnings("deprecation")
  public List<TransactionStatisticsTO> getRobotTransactionStatistics(String minDate, String maxDate)
      throws ParseException {
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    ...
    List<TransactionStatisticsTO> result = new ArrayList<TransactionStatisticsTO>();
    for (Object[] objects :
        new RobotTransactionDAO(getSession()).findForDates(partnerId, minD, maxD)) {
  ...
  private void addTransactionStatResult(
      List<TransactionStatisticsTO> result, Long count, String message, Double price) {
    if (price != null) {
      message += " " + price;
