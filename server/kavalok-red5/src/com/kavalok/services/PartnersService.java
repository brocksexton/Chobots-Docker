...
public class PartnersService extends DataServiceNotTransactionBase {
  ...
  public PagedResult<UserTO> getUsersByDates(
      Date from, Date to, Integer firstResult, Integer maxResults) {
    Partner partner = getPartner();
    List<User> users =
        new UserDAO(getSession()).findByPartner(partner, from, to, firstResult, maxResults);
    List<UserTO> tos = UserTO.convertUsers(getSession(), users);
    return new PagedResult<UserTO>(
        new UserDAO(getSession()).sizeByReferrer(partner, from, to), tos);
  }
