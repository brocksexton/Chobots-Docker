...
public class CompetitionService extends DataServiceNotTransactionBase {
  ...
  public Byte[] addCompetitorResult(
      LinkedHashMap competitorUserIdEnc,
      LinkedHashMap competitionNameEnc,
    ...
    if (competitionOpen(competition)) {
      ...
      User competitor =
          competitorId != null
              ? userDAO.findById(competitorId)
              : userDAO.findByLogin(competitorLogin);
      Integer count =
          competitionResultDAO.countByCompetitors(user, competitor, competition).intValue();
      Double newResult = result / (count + 1);
      CompetitionResult competitionResult =
          new CompetitionResult(user, competitor, competition, newResult);
  ...
  private boolean competitionOpen(Competition competition) {
    ...
    return competition != null
        && competition.getStart().before(now)
        && competition.getFinish().after(now);
  }
}
