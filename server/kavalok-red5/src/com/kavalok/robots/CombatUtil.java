...
public class CombatUtil {
  ...
  public static void applyEnergy(CombatPlayer source, CombatPlayer target) {
  ...
  public static void applyCombat(CombatPlayer source, CombatPlayer target) {
  ...
  private static void applyBaseAttack(CombatPlayer source, CombatPlayer target) {
    ...
    int randomAttack =
        getRandomAttack(
            sourceAttack, source.getRobot().getAccuracy(), target.getRobot().getMobility());
  ...
  private static void applySpecialAttack(CombatPlayer source, CombatPlayer target) {
    ...
    int sourceAttack = 0;
    if (item.getAttack() > 0) {
      if (!item.getPercent()) sourceAttack = item.getAttack();
      else sourceAttack = (int) (target.getRobot().getMaxEnergy() * item.getAttack() / 100.0);
    }
