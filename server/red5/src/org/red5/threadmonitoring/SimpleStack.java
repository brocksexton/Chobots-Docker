 ...
 * Besides common stack operations like <b>pop </b>, <b>push </b> it is easy to retrieve each
...
public class SimpleStack {
   ...
   * If negative size is passed, stack is initialized with default initial capasity, equals 10.
   *
   * @param size initial capasity of the stack being created.
   */
   ...
   * Before storing element in the stack, method {@link #ensureCapacity(int) ensureCapacity}is
   * called to ensure that there is enough space to store new element.
   *
   * @param element new element is being pushed to the stack.
   */
   ...
   * If stack is empty {@link ArrayIndexOutOfBoundsException ArrayIndexOfBoundException}is thrown.
   * <br>
   * Method does not affect the array, it just decrement stack size value.
   *
   * @return element on the top of stack.
   * @throws EmptyStackException if operation is proceed on empty stack.
   */
  public long pop() {
    if (size == 0) {
   ...
   * The top element of stack has the biggest position equal to {@link #size() size()}- 1.<br>
   * If position < 0 || position >= SimpleStack#size(), IndexOutOfBoundsException is thrown.
   *
   * @param pos position of the element to retrieve.
   * @return element of the stack at position <code>pos</code>
   * @throws IndexOutOfBoundsException if operation is executed on an empty stack.
   */
