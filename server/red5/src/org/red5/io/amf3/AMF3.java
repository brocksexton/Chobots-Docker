 ...
 *
 * <p>For detailed specification please see the link below.
 *
...
public class AMF3 {
   ...
   *
   * <p>The remaining integer-data represents the number of class members that exist. The property
   * names are read as string-data. The values are then read as AMF3-data.
   */
   ...
   *
   * <p>What follows is the value of the "inner" object, including type code. This value appears for
   * objects that implement IExternalizable, such as ArrayCollection and ObjectProxy.
   */
   ...
   * Name-value encoding.
   *
   * <p>The property names and values are encoded as string-data followed by AMF3-data until there
   * is an empty string property name. If there is a class-def reference there are no property names
   * and the number of values is equal to the number of properties in the class-def.
   */
