...
public class HexCharset extends Charset {
  ...
  public class Decoder extends CharsetDecoder {
    ...
    private Decoder() {
      super(
          HexCharset.this, 2.0F, measure != null ? 2.0F + 2.0F / (float) measure.intValue() : 2.0F);
    }
        ...
        if (!Character.isWhitespace(inch)) {
          ...
          if (d < 0)
            throw new IllegalArgumentException(
                (new StringBuilder()).append("Bad hex character ").append(inch).toString());
  ...
  public HexCharset(boolean caps, int measure) {
    super(
        (new StringBuilder()).append(caps ? "HEX" : "hex").append(":").append(measure).toString(),
        new String[] {"HEX"});
