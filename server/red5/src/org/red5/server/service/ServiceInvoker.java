...
public class ServiceInvoker implements IServiceInvoker {
  ...
  public boolean invoke(IServiceCall call, Object service) {
    ...
    methodResult =
        ServiceUtils.findMethodWithExactParameters(service, methodName, argsWithConnection);
    if (methodResult.length == 0 || methodResult[0] == null) {
      ...
      if (methodResult.length == 0 || methodResult[0] == null) {
        ...
        methodResult =
            ServiceUtils.findMethodWithListParameters(service, methodName, argsWithConnection);
        if (methodResult.length == 0 || methodResult[0] == null) {
          ...
          if (methodResult.length == 0 || methodResult[0] == null) {
            ...
            //									(args == null ? Collections.EMPTY_LIST : Arrays.asList(args)) + " not found
            // in " + service);
            //						}
