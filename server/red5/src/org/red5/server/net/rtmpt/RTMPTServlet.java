...
public class RTMPTServlet extends HttpServlet {
  ...
  /**
   * Try to generate responses that contain at least 32768 bytes data. Increasing this value results
   * in better stream performance, but also increases the latency.
   */
  ...
  protected void returnPendingMessages(RTMPTConnection client, HttpServletResponse resp)
    ...
    if (data == null) {
      ...
      else returnMessage(client.getPollingDelay(), resp);
      return;
    }
  ...
  protected RTMPTConnection createConnection() {
    RTMPTConnection conn =
        (RTMPTConnection) rtmpConnManager.createConnection(RTMPTConnection.class);
