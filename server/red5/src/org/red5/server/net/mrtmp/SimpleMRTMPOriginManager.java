...
public class SimpleMRTMPOriginManager implements IMRTMPOriginManager {
  ...
  public SimpleMRTMPOriginManager() {
    ...
    // integration.
    clientToConnMap = Collections.synchronizedMap(new HashMap<RTMPConnection, IMRTMPConnection>());
  }
  ...
  public boolean unregisterConnection(IMRTMPConnection conn) {
    ...
    try {
      ...
      if (ret) {
        for (Iterator<Entry<RTMPConnection, IMRTMPConnection>> iter =
                clientToConnMap.entrySet().iterator();
            iter.hasNext(); ) {
    ...
    for (RTMPConnection rtmpConn : list) {
      log.debug(
          "Close RTMPOriginConnection " + rtmpConn.getId() + " due to MRTMP Connection closed!");
