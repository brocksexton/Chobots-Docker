...
public interface IMRTMPManager {
  /**
   * Map a client to an Edge/Origin MRTMP connection. On Edge, the server will find an Origin
   * connection per routing logic. On Origin, the server will send to the original in-coming
   * connection if the client connection type is persistent. Or the latest in-coming connection will
   * be used.
   *
   * @param conn
   * @return
