...
public interface StatusCodes {
   ...
   * 'rtmp' must be specified when connecting to FMS and Red5. Either not supported version of AMF
   * was used (3 when only 0 is supported)
   */
  ...
  /**
   * The client does not have permission to connect to the application, the application expected
   * different parameters from those that were passed, or the application name specified during the
   * connection attempt was not found on the server.
   */
  ...
  /**
   * The subscriber has used the seek command to move to a particular location in the recorded
   * stream.
   */
   ...
   *
   * <p>filename: name of the offending ASC file. lineno: line number where the error occurred.
   * linebuf: source code of the offending line.
   */
  public static final String APP_SCRIPT_ERROR = "Application.Script.Error";
  /**
   * The ActionScript engine has encountered a runtime warning. In addition to the standard
   * infoObject properties, the following properties are set:
   *
   * <p>filename: name of the offending ASC file. lineno: line number where the error occurred.
   * linebuf: source code of the offending line
   */
   ...
   * application instance to free some resources or take suitable action. If the application
   * instance runs out of memory, it is unloaded and all users are disconnected. In this state, the
   * server will not invoke the Application.onDisconnect event handler or the Application.onAppStop
   * event handler
   */
  public static final String APP_RESOURCE_LOWMEMORY = "Application.Resource.LowMemory";
  /**
   * This information object is passed to the onAppStop handler when the application is being shut
   * down
   */
  ...
  /**
   * This event is sent if the player detects an MP4 with an invalid file structure. Flash Player
  ...
  /**
   * This event is sent if the player does not detect any supported tracks. If there aren't any
   * supported video, audio or data tracks found, Flash Player does not play the file.
   *
   * <p>New for FMS3
   */
  public static final String NS_PLAY_NO_SUPPORTED_TRACK_FOUND =
