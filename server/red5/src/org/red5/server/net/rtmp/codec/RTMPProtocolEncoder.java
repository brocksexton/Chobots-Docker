...
public class RTMPProtocolEncoder extends BaseProtocolEncoder
  ...
  /** Logger. */
  protected static Logger log = LoggerFactory.getLogger(RTMPProtocolEncoder.class.getName());

  /** I/O operations logger. */
  protected static Logger ioLog =
      LoggerFactory.getLogger(RTMPProtocolEncoder.class.getName() + ".out");
  ...
  public ByteBuffer encode(ProtocolState state, Object message) throws Exception {
    ThreadMonitorServices.setJobDetails(
        "RTMPProtocolEncoder.encode(ProtocolState state, Object message)");
    try {
  ...
  public ByteBuffer encodePacket(RTMP rtmp, Packet packet) {
    ...
    final int bufSize =
        header.getSize() + headerSize + (numChunks > 0 ? (numChunks - 1) * chunkHeaderSize : 0);
  ...
  private int calculateHeaderSize(Header header, Header lastHeader) {
    ...
    int channelIdAdd;
    if (header.getChannelId() > 320) channelIdAdd = 2;
    else if (header.getChannelId() > 63) channelIdAdd = 1;
    else channelIdAdd = 0;
    ...
    if (!isPending) {
      ...
      final String action =
          (call.getServiceName() == null)
              ...
              : call.getServiceName() + '.' + call.getServiceMethodName();
