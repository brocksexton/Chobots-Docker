...
public class RTMPProtocolDecoder implements Constants, SimpleProtocolDecoder, IEventDecoder {
  ...
  /** Logger. */
  protected static Logger log = LoggerFactory.getLogger(RTMPProtocolDecoder.class.getName());

  /** I/O logger. */
  protected static Logger ioLog =
      LoggerFactory.getLogger(RTMPProtocolDecoder.class.getName() + ".in");
  ...
  public Object decode(ProtocolState state, ByteBuffer in) throws ProtocolException {
    ThreadMonitorServices.setJobDetails(
        "RTMPProtocolEncoder.decode(ProtocolState state, ByteBuffer in)");
  ...
  public Packet decodePacket(RTMP rtmp, ByteBuffer in) {
    ...
    if ((headerByte & 0x3f) == 0) {
      ...
      headerValue =
          ((int) headerByte & 0xff) << 16 | ((int) in.get() & 0xff) << 8 | ((int) in.get() & 0xff);
      byteCount = 3;
    } else {
      // Single byte header
    ...
    int headerLength =
        RTMPUtils.getHeaderLength(RTMPUtils.decodeHeaderSize(headerValue, byteCount));
    ...
    if (log.isWarnEnabled()) {
      ...
      if (buf.position() > header.getSize() + addSize) {
        log.warn(
            "Packet size expanded from "
                + (header.getSize() + addSize)
                + " to "
                + buf.position()
                + " ("
                + header
   ...
   * @return <code>true</code> if passed action is a reserved stream method, <code>false</code>
   *     otherwise
   */
  ...
  protected Notify decodeNotifyOrInvoke(Notify notify, ByteBuffer in, Header header, RTMP rtmp) {
    ...
    if (!(notify instanceof Invoke)
        && rtmp != null
        && rtmp.getMode() == RTMP.MODE_SERVER
        && header != null
        && header.getStreamId() != 0
        && !isStreamCommand(action)) {
