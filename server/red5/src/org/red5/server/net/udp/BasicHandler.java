...
/**
 * => client send to server <= server send to client << server broadcast
 *
 * <p>Connecting to the server
 *
 * <p>=> byte(join) << byte(join) int(id) => byte(list) <= byte(list) int(count) int(id) int(id) ...
 *
 * <p>Sending a message to all
 *
 * <p>=> byte(send) [..anything..] << byte(send) [..anything..]
 *
 * <p>Server ping client to keep alive, every second
 *
 * <p><= byte(noop) => byte(noop)
 *
 * <p>Timeouts (after 10s no reply)
 *
 * <p><< byte(exit) int(id)
 *
 * @author luke
...
public class BasicHandler extends IoHandlerAdapter {
  ...
  protected Timer timer = new Timer("BasicHandler", true);
