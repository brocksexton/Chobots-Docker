...
public class ScopeUtils {
  ...
  /**
   * Finds root scope for specified scope object. Root scope is the top level scope among scope's
   * parents.
   *
  ...
  /**
   * Returns the application scope for specified scope. Application scope has depth of 1 and has no
   * parent.
   *
   * <p>See <code>isApp</code> method for details.
   *
   ...
   * @return <code>true</code> if ancestor scope is really an ancestor of scope passed as from
   ...
   * @return <code>true</code> if scope is root scope (top level scope), <code>false</code>
   *     otherwise.
   */
   ...
   *
   * <p>When user connects the following URL: rtmp://localhost/myapp/foo/bar then / is the global
   * level scope, myapp is app level, foo is room level and bar is room level as well (but with
   * higher depth level)
   *
   * @param scope Scope to check
  ...
  /**
   * Check whether scope is a room scope (level 2 leaf in scope tree or lower, e.g. 3, 4, ...) or
   * not
   *
   * @param scope Scope to check
  ...
  /**
   * Returns scope services (e.g. SharedObject, etc) for the scope. Method uses either bean name
   * passes as a string or class object.
   *
  ...
  public static Object getScopeService(
      IScope scope, Class<?> intf, Class<?> defaultClass, boolean checkHandler) {
    ...
    if (scope.hasAttribute(
        IPersistable.TRANSIENT_PREFIX + SERVICE_CACHE_PREFIX + intf.getCanonicalName())) {
      // Return cached service
      return scope.getAttribute(
          IPersistable.TRANSIENT_PREFIX + SERVICE_CACHE_PREFIX + intf.getCanonicalName());
    }
    ...
    if (handler == null && IScopeService.class.isAssignableFrom(intf)) {
      ...
      try {
        ...
        if (!(serviceName instanceof String)) serviceName = null;
      } catch (Exception e) {
        log.debug("No string field 'BEAN_NAME' in that interface");
      }

      if (serviceName != null) handler = getScopeService(scope, (String) serviceName, defaultClass);
    }
    ...
    // Cache service
    scope.setAttribute(
        IPersistable.TRANSIENT_PREFIX + SERVICE_CACHE_PREFIX + intf.getCanonicalName(), handler);
    return handler;
  }
}
