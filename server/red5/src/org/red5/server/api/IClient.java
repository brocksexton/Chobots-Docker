...
/**
 * The client object represents a single client. One client may have multiple connections to
 * different scopes on the same host. In some ways the client object is like a HTTP session. You can
 * create IClient objects with {@link IClientRegistry#newClient(Object[])}
 *
 * <p>NOTE: I removed session, since client serves the same purpose as a client with attributes
 *
 * @author The Red5 Project (red5@osflash.org)
 * @author Luke Hubbard (luke@codegent.com)
 */
public interface IClient extends IAttributeStore, IBWControllable {

  /** The key used to store the client object in a http session. */
  public static final String ID = "red5.client";

  /**
   * Get the unique ID for this client. This will be generated by the server if not passed upon
   * connection from client-side Flex/Flash app. To assign a custom ID to the client use <code>
   * params</code> object of {@link IApplication#appConnect(IConnection, Object[])} method, that
   * contains 2nd all the rest values you pass to <code>NetConnection.connect</code> method.
   *
   * <p>Example:
   *
   * <p>At client side: <code>NetConnection.connect( "http://localhost/killerapp/", "user123" );
   * </code> then at server side: <code>
   ...
   * @param permissions Permissions the client has in this context or <code>null</code> for no
   *     permissions.
   */
