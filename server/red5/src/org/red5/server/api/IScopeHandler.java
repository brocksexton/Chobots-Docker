...
/**
 * The scope handler controls actions performed against a scope object, and also is notified of all
 * events.
 *
 * <p>Gives fine grained control over what actions can be performed with the can* methods. Allows
 * for detailed reporting on what is happening within the scope with the on* methods. This is the
 * core interface users implement to create applications.
 *
 * <p>The thread local connection is always available via the Red5 object within these methods
 *
...
public interface IScopeHandler extends IEventHandler {
  ...
  /**
   * Called just before every connection to a scope. You can pass additional params from client
   * using <code>NetConnection.connect</code> method (see below).
   *
   * @param conn Connection object
   * @param params List of params passed from client via <code>NetConnection.connect</code> method.
   *     All parameters but the first one passed to <code>NetConnection.connect</code> method are
