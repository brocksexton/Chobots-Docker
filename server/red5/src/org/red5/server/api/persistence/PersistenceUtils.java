...
public class PersistenceUtils {
   ...
   * @throws Exception
   */
  private static Constructor<?> getPersistenceStoreConstructor(
      Class<?> theClass, Class<?>[] interfaces) throws Exception {
  ...
  /**
   * Returns persistence store object. Persistence store is a special object that stores persistence
   * objects and provides methods to manipulate them (save, load, remove, list).
   *
  ...
  public static IPersistenceStore getPersistenceStore(
      ResourcePatternResolver resolver, String className) throws Exception {
    Class<?> persistenceClass = Class.forName(className);
    Constructor<?> constructor =
        getPersistenceStoreConstructor(persistenceClass, resolver.getClass().getInterfaces());
    if (constructor == null) {
