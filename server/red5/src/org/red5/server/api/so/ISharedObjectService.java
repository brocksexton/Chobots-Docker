...
public interface ISharedObjectService extends IScopeService {
  ...
  /**
   * Deletes persistent shared objects specified by name and clears all properties from active
   * shared objects (persistent and nonpersistent). The name parameter specifies the name of a
   * shared object, which can include a slash (/) as a delimiter between directories in the path.
   * The last element in the path can contain wildcard patterns (for example, a question mark [?]
   * and an asterisk [*]) or a shared object name. The clearSharedObjects() method traverses the
   * shared object hierarchy along the specified path and clears all the shared objects. Specifying
   * a slash (/) clears all the shared objects associated with an application instance.
   *
   * <p>The following values are possible for the soPath parameter: <br>
   ...
   * cleared.
   *
   * <p>If you call the clearSharedObjects() method and the specified path matches a shared object
   * that is currently active, all its properties are deleted, and a "clear" event is sent to all
   * subscribers of the shared object. If it is a persistent shared object, the persistent store is
   * also cleared. <br>
   *
   * @param scope the scope to check for the shared object
   * @param name the name of the shared object
   * @return true if the shared object at the specified path was deleted; otherwise, false. If using
   *     wildcard characters to delete multiple files, the method returns true only if all the
   *     shared objects matching the wildcard pattern were successfully deleted; otherwise, it will
   *     return false.
   */
