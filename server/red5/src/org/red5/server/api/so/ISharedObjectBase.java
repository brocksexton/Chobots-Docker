...
/**
 * Base interface for shared objects. Changes to the shared objects are propagated to all subscribed
 * clients.
 *
 * <p>If you want to modify multiple attributes and notify the clients about all changes at once,
 ...
 * </code> All changes between "beginUpdate" and "endUpdate" will be sent to the clients using one
 * notification event.
 *
...
public interface ISharedObjectBase extends ISharedObjectHandlerProvider, ICastingAttributeStore {
  ...
  /**
   * Returns the version of the shared object. The version is incremented automatically on each
   * modification.
   *
  ...
  /**
   * Detaches a reference from this shared object, this will destroy the reference immediately. This
   * is useful when you don't want to proxy a shared object any longer.
   */
  public void close();
}
