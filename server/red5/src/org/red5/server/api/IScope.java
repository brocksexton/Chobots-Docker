 ...
 * The scope object.
 *
 * <p>A statefull object shared between a group of clients connected to the same <tt>context
 * path</tt>. Scopes are arranged in hierarchical way, so its possible for a scope to have a parent
 * and children scopes. If a client connects to a scope then they are also connected to its parent
 * scope. The scope object is used to access resources, shared object, streams, etc. That is, scope
 * are general option for grouping things in application.
 *
 * <p>The following are all names for scopes: application, room, place, lobby.
 *
...
public interface IScope extends IBasicScope, ResourcePatternResolver, IServiceHandlerProvider {
  ...
  /**
   * Creates child scope with name given and returns success value. Returns <code>true</code> on
   * success, <code>false</code> if given scope already exists among children.
   *
  ...
  /**
   * Adds scope as a child scope. Returns <code>true</code> on success, <code>false</code> if given
   * scope is already a child of current.
   *
  ...
  /**
   * Removes scope from the children scope list. Returns <code>false</code> if given scope isn't a
  ...
  /**
   * Get a set of connected clients. You can get the connections by passing the scope to the clients
   * {@link IClient#getConnections()} method.
   *
   ...
   * @return <code>true</code> on success, <code>false</code> if given connection already belongs to
   *     this scope
   */
  ...
  /**
   * Removes given connection from list of scope connections. This disconnects all clients of given
   * connection from the scope.
   *
