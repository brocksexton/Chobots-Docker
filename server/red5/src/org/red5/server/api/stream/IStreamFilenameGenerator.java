...
public interface IStreamFilenameGenerator extends IScopeService {
   ...
   *
   * <p>If relative to application, you need to use <code>
   * scope.getContext().getResources(fileName)[0].getFile()</code> to resolve this to a file.
   *
   * <p>If absolute (ie returns true) simply use <code>new File(generateFilename(scope, name))
   * </code>
   *
   * @return
   */
