 ...
 *
 * <p>Each connection has an associated client and scope. Connections may be persistent, polling, or
 * transient. The aim of this interface is to provide basic connection methods shared between
 * different types of connections
 *
 * <p>Future subclasses: RTMPConnection, RemotingConnection, AJAXConnection, HttpConnection, etc
 *
...
public interface IConnection extends ICoreObject {
  ...
  /**
   * Is the client connected to the scope. Result depends on connection type, <code>true</code> for
   * persistent and polling connections, <code>false</code> for transient.
   *
   * @return <code>true</code> if the connection is persistent or polling, otherwise <code>false
   *     </code>
   */
  ...
  /**
   * Get the hostname that the client is connected to. If they are connected to an IP, the IP
   * address will be returned as a String.
   *
  ...
  /**
   * Get the IP addresses the client is connected from. If a client is connected through RTMPT and
   * uses a proxy to connect, this will contain all hosts the client used to connect to the server.
   *
  ...
  /**
   * Return number of written bytes the client reports to have received. This is the last value of
   * the BytesRead message received from a client.
   *
  ...
  /**
   * Get the basic scopes this connection has subscribed. This list will contain the shared objects
   * and broadcast streams the connection connected to.
   *
