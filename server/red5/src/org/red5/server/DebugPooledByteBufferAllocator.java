 ...
 *
 * <p>All buffers are allocated with the size of power of 2 (e.g. 16, 32, 64, ...) This means that
 * you cannot simply assume that the actual capacity of the buffer and the capacity you requested
 * are same.
 *
 * <p>This allocator releases the buffers which have not been in use for a certain period. You can
 * adjust the period by calling {@link #setTimeout(int)}. The default timeout is 1 minute (60
 * seconds). To release these buffers periodically, a daemon thread is started when a new instance
 * of the allocator is created. You can stop the thread by calling {@link #dispose()}.
 *
...
public class DebugPooledByteBufferAllocator implements ByteBufferAllocator {
  ...
  protected HashMap<UnexpandableByteBuffer, StackTraceElement[]> stacks =
      new HashMap<UnexpandableByteBuffer, StackTraceElement[]>();

  /**
   * Save a stack trace for every buffer allocated?
   *
   * <p>Warning: This slows down the Red5 a lot!
   */
  ...
  private UnexpandableByteBuffer allocate0(int capacity, boolean direct) {
    ...
    if (buf == null) {
      java.nio.ByteBuffer nioBuf =
          direct
              ? java.nio.ByteBuffer.allocateDirect(MINIMUM_CAPACITY << idx)
  ...
  private class PooledByteBuffer extends ByteBuffer {
    ...
    public ByteBuffer duplicate() {
      ...
      newBuf.init(new UnexpandableByteBuffer(buf.buf().duplicate(), buf), false);
      return newBuf;
    }
    ...
    public ByteBuffer slice() {
      PooledByteBuffer newBuf = allocateContainer();
      newBuf.init(new UnexpandableByteBuffer(buf.buf().slice(), buf), false);
      return newBuf;
    }
    ...
    public ByteBuffer asReadOnlyBuffer() {
      PooledByteBuffer newBuf = allocateContainer();
      newBuf.init(new UnexpandableByteBuffer(buf.buf().asReadOnlyBuffer(), buf), false);
      return newBuf;
    }
