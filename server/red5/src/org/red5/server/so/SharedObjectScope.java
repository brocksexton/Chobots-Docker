...
public class SharedObjectScope extends BasicScope implements ISharedObject, StatusCodes {
  ...
  private Set<ISharedObjectListener> serverListeners =
      new CopyOnWriteArraySet<ISharedObjectListener>();
  ...
  private Set<ISharedObjectSecurity> securityHandlers =
      new CopyOnWriteArraySet<ISharedObjectSecurity>();
   ...
   */
  public SharedObjectScope(
      IScope parent, String name, boolean persistent, IPersistenceStore store) {
  ...
  public void beginUpdate() {
    ...
    ThreadMonitorServices.setJobDetails("SharedObjectScope.beginUpdate()");
    // Make sure only one thread can update the SO
    lock.lock();
    so.beginUpdate();
    ThreadMonitorServices.stopJobSubDetails("SharedObjectScope.beginUpdate()");
  }
  ...
  public void beginUpdate(IEventListener listener) {
    ...
    ThreadMonitorServices.setJobDetails(
        "SharedObjectScope.beginUpdate(IEventListener listener) {0}", listener);
    // Make sure only one thread can update the SO
    lock.lock();
  ...
  public void endUpdate() {
    ThreadMonitorServices.setJobDetails("SharedObjectScope.endUpdate()");
    // End update of SO
    try {
      so.endUpdate();
    } catch (Throwable e) {
    ...
    ThreadMonitorServices.stopJobSubDetails("SharedObjectScope.endUpdate()");
  }
  ...
  public void sendMessage(String handler, List arguments) {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails(
        "SharedObjectScope.sendMessage(String handler {0}, List arguments {1})",
        handler, arguments);
    // counting chat time
    ...
    try {
      ...
      for (ISharedObjectListener listener : serverListeners) {
        listener.beforeSharedObjectSend(this, handler, arguments);
      }
      so.sendMessage(handler, arguments);
    } finally {
      endUpdate();
    }
    if (arguments.toString().indexOf("L, C,") >= 0) {
      System.out.println("message arguments: " + arguments);
      System.out.println("sendMessage time: " + (System.currentTimeMillis() - now));
    }
    ...
    if (soHandler == null && hasParent()) {
      ...
      try {
        ...
        if (!so.getName().startsWith("loc")
            && !so.getName().startsWith("farm")
            && !so.getName().startsWith("admin_shared")
            && !so.getName().startsWith("|game")
            && !so.getName().startsWith("game")
            && !so.getName().startsWith("home")
            && !so.getName().startsWith("mission"))
    ...
    if (soHandler != null) {
      // With exact params...
      Object[] methodResult =
          ServiceUtils.findMethodWithExactParameters(soHandler, serviceMethod, arguments);
      // Or at least with suitable list params
      if (methodResult.length == 0 || methodResult[0] == null) {
        methodResult =
            ServiceUtils.findMethodWithListParameters(soHandler, serviceMethod, arguments);
      }
      ...
      if (methodResult.length > 0 && methodResult[0] != null) {
        ...
        try {
          ThreadMonitorServices.setJobDetails("handler invoke");
          method.invoke(soHandler, params);
        } catch (Exception err) {
          log.error(
              "Error while invoking method "
                  + serviceMethod
                  + " on shared object handler "
                  + handler,
              err);
        }
      }
  ...
  public void removeEventListener(IEventListener listener) {
    ...
    try {
      ...
      if (!so.isPersistentObject() && (so.getListeners() == null || so.getListeners().isEmpty())) {
        ...
        for (ISharedObjectListener soListener : serverListeners)
          soListener.onSharedObjectDestroy(this);
      }
    }
  }
  ...
  private Set<ISharedObjectSecurity> getSecurityHandlers() {
    ISharedObjectSecurityService security =
        (ISharedObjectSecurityService)
            ScopeUtils.getScopeService(getParent(), ISharedObjectSecurityService.class);
  ...
  /**
   * Locks the shared object instance. Prevents any changes to this object by clients until the
   * SharedObject.unlock() method is called.
   */
  public void lock() {
    lock.lock();
  }
