 ...
 * on client side. In Red5 and Flash Media Server there's one more special type of SOs : remote
 * Shared Objects.
 *
 * <p>These are shared by multiple clients and synchronized between them automatically on each data
 * change. This is done asynchronously, used as events handling and is widely used in multiplayer
 * Flash online games.
 *
 * <p>Shared object can be persistent or transient. The difference is that first are saved to the
 * disk and can be accessed later on next connection, transient objects are not saved and get lost
 * each time they last client disconnects from it.
 *
 * <p>Shared Objects has name identifiers and path on server's HD (if persistent). On deeper level
 * server-side Shared Object in this implementation actually uses IPersistenceStore to delegate all
 * (de)serialization work.
 *
 * <p>SOs store data as simple map, that is, "name-value" pairs. Each value in turn can be complex
 * object or map.
 *
 * <p>All access to methods that change properties in the SO must be properly synchronized for
 * multi-threaded access.
 */
public class SharedObject extends AttributeStore
  ...
  /**
   * true if the SharedObject was stored by the persistence framework (NOT in database, just plain
   * serialization to the disk) and can be used later on reconnection
   */
  ...
  protected void sendUpdates() {
    ThreadMonitorServices.setJobDetails("SO sendUpdates() ownerMessage" + ownerMessage);
    if (!ownerMessage.getEvents().isEmpty()) {

      // for (Iterator<ISharedObjectEvent> iterator =
      // ownerMessage.getEvents().iterator(); iterator.hasNext();) {
      // ISharedObjectEvent evt = iterator.next();
      // if (evt.toString().indexOf("[L, M, {") > 0) {
      // ownerMessage.getEvents().remove(evt);
      // break;
      // }
      // }
      ...
      SharedObjectMessage syncOwner =
          new SharedObjectMessage(null, name, version, isPersistentObject());
    ...
    if (!syncEvents.isEmpty()) {
      ...
      // for (Iterator<ISharedObjectEvent> iterator = syncEvents.iterator();
      // iterator.hasNext();) {
      // ISharedObjectEvent evt = iterator.next();
      // if (evt.toString().indexOf("[L, M, {") > 0 ) {
      // syncEvents.remove(evt);
      // break;
      // }
      // }
      ...
      for (IEventListener listener : listeners) {
        ...
        ChannelWorker cworker = new ChannelWorker(c, syncMessage);
  ...
  protected void notifyModified() {
    ThreadMonitorServices.setJobDetails("SharedObject.notifyModified()");
    if (updateCounter > 0) {
   ...
   * @return <code>true</code> if there's such attribute and value was set, <code>false</code>
   *     otherwise
   */
  @Override
   ...
   * @return <code>true</code> if there's such an attribute and it was removed, <code>false</code>
   *     otherwise
   */
  @Override
  ...
  protected void sendMessage(String handler, List<?> arguments) {
    ThreadMonitorServices.setJobDetails(
        "SharedObject.sendMessage(String handler {0}, List<?> arguments {1})", handler, arguments);
    // Forward
  ...
  protected void register(IEventListener listener) {
    ...
    ThreadMonitorServices.setJobDetails(
        "SharedObject.register(IEventListener listener {0})", listener);
  ...
  protected void checkRelease() {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails("SharedObject.checkRelease()");
    ...
    ThreadMonitorServices.stopJobSubDetails("SharedObject.checkRelease()");
  }
  ...
  protected void unregister(IEventListener listener) {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails(
        "SharedObject.unregister(IEventListener listener {0})", listener);
  ...
  /**
   * End update of this Shared Object. Decreases number of pending update operations and broadcasts
   * modified event if it is equal to zero (i.e. no more pending update operations).
   */
  protected void endUpdate() {
    ThreadMonitorServices.setJobDetails("SharedObject.endUpdate()");
  ...
  /**
   * Deletes all the attributes and sends a clear event to all listeners. The persistent data object
   * is also removed from a persistent shared object.
   *
  ...
  /**
   * Detaches a reference from this shared object, reset it's state, this will destroy the reference
   * immediately. This is useful when you don't want to proxy a shared object any longer.
   */
  protected void close() {
    ThreadMonitorServices.startJobSubDetails();
    ThreadMonitorServices.setJobDetails("SharedObject.close()");
    // clear collections
    super.removeAttributes();
    listeners.clear();
    syncEvents.clear();
    ownerMessage.getEvents().clear();
    ThreadMonitorServices.stopJobSubDetails("SharedObject.close()");
  }

  /**
   * Prevent shared object from being released. Each call to <code>acquire</code> must be paired
   * with a call to <code>release</code> so the SO isn't held forever. This is only valid for
   * non-persistent SOs.
   */
  ...
  public synchronized void release() {
    if (acquireCount == 0) throw new RuntimeException("The shared object was not acquired before.");

    acquireCount--;
    if (acquireCount == 0) checkRelease();
  }
  ...
  private static final class ChannelWorker implements Runnable {
    private Channel channel;

    private SharedObjectMessage message;

    @Override
    public void run() {
      ThreadMonitorServices.start("ChannelWorker.run()");
      ThreadMonitorServices.setJobType("channel-worker");
      log.debug("Send to {}", channel);
      channel.write(message);
      ThreadMonitorServices.finish();
    }

    public ChannelWorker(Channel channel, SharedObjectMessage message) {
