 ...
 * The scope object.
 *
 * <p>A statefull object shared between a group of clients connected to the same context path.
 * Scopes are arranged in a hierarchical way, so its possible for a scope to have a parent. If a
 * client is connect to a scope then they are also connected to its parent scope. The scope object
 * is used to access resources, shared object, streams, etc.
 *
...
public class Scope extends BasicScope implements IScope, IScopeStatistics {
  ...
  private final ConcurrentMap<String, IBasicScope> children =
      new ConcurrentHashMap<String, IBasicScope>();

  /** Clients and connection map */
  private final ConcurrentMap<IClient, Set<IConnection>> clients =
      new ConcurrentHashMap<IClient, Set<IConnection>>();
  ...
  /**
   * Registered service handlers for this scope. The map is created on-demand only if it's accessed
   * for writing.
   */
   ...
   * @return <code>true</code> on success (if scope has handler and it accepts child scope
   *     addition), <code>false</code> otherwise
   */
  ...
  /**
   * Return set of service handler names. Removing entries from the set unregisters the
   ...
   * @param name Child scope name
   * @return <code>true</code> if scope has child node with given name and type, <code>false</code>
   *     otherwise
   */
   ...
   *
   * @return <code>true</code> if scope or it's parent scope has a handler, <code>false</code>
   *     otherwise
   */
   ...
   * Starts scope
   *
   * @return <code>true</code> if scope has handler and it's start method returned true, <code>false
   *     </code> otherwise
   */
