...
public class ServerStream extends AbstractStream
  ...
  public void saveAs(String name, boolean isAppend)
    ...
    try {
      ...
      IStreamFilenameGenerator generator =
  ...
  public void start() {
    ...
    IProviderService providerService =
        (IProviderService) getScope().getContext().getBean(IProviderService.BEAN_NAME);
  ...
  /**
   * Pipe connection event handler. There are two types of pipe connection events so far, provider
   * push connection event and provider disconnection event.
   *
   * <p>Pipe events handling is the most common way of working with pipes.
   *
  ...
  /**
   * Play a specific IPlayItem. The strategy for now is VOD first, Live second. Should be called in
  ...
  /**
   * Move to the previous item updating the currentItemIndex. Should be called in synchronized
   * context.
   */
