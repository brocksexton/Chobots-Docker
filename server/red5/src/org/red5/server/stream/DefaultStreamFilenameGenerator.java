...
/**
 * Default filename generator for streams. The files will be stored in a directory "streams" in the
 * application folder. Option for changing directory streams are saved to is investigated as of
 * 0.6RC1.
 *
...
public class DefaultStreamFilenameGenerator implements IStreamFilenameGenerator {
  ...
  /**
   * Generate stream directory based on relative scope path. The base directory is <code>streams
   * </code>, e.g. a scope <code>/application/one/two/</code> will generate a directory <code>
   * /streams/one/two/</code> inside the application.
   *
   * @param scope Scope
