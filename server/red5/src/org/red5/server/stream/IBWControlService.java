 ...
 *
 * <p>The bandwidth controllable is registered in the bandwidth controller which provides the three
 * token buckets used for bandwidth control.
 *
 * <p>The bandwidth controller manages the token buckets assigned to the bandwidth controllable and
 * distributes the tokens to the buckets in an implementation-specific way. (eg timely distribute
 * the tokens according to the bandwidth config of the controllable)
 *
...
public interface IBWControlService {
  ...
  /**
   * Register a bandwidth controllable. The necessary resources will be allocated and assigned to
   * the controllable.
   *
  ...
  /**
   * Update the bandwidth configuration of a controllable. Each time when the controllable changes
   * the bandwidth config and wants to make the changes take effect, this method should be called.
   *
   * @param context The registry context.
   */
  ...
  /**
   * Reset all the token buckets for a controllable. All the callback will be reset and all blocked
