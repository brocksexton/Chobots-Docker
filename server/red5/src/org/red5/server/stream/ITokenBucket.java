...
public interface ITokenBucket {
  /**
   * Acquire tokens amount of <tt>tokenCount</tt> waiting <tt>wait</tt> milliseconds if token not
   * available.
   *
   * @param tokenCount The count of tokens to acquire.
   * @param wait Milliseconds to wait. <tt>0</tt> means no wait and any value below zero means wait
   *     forever.
  ...
  /**
   * Nonblockingly acquire token. If the token is not available and <tt>task</tt> is not null, the
   * callback will be executed when the token is available. The tokens are not consumed
   * automatically before callback, so it's recommended to acquire token again in callback function.
   *
  ...
  /**
   * Reset this token bucket. All pending threads are woken up with <tt>false</tt> returned for
   * acquiring token and callback is removed without calling back.
   */
  void reset();
