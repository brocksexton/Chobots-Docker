 ...
 */
public class ClientBroadcastStream extends AbstractClientStream
  ...
  /**
   * Closes stream, unsubscribes provides, sends stoppage notifications and broadcast close
   * notification.
   */
  public void close() {
    if (closed) {
   ...
   *
   * @param name Name that used for publishing. Set at client side when begin to broadcast with
   *     NetStream#publish.
   */
  ...
  public void start() {
    ...
    try {
      videoCodecFactory =
          (VideoCodecFactory) getScope().getContext().getBean(VideoCodecFactory.KEY);
