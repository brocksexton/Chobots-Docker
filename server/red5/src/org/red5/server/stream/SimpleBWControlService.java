...
/**
 * A simple implementation of bandwidth controller. The initial burst, if not specified by user, is
 * half of the property "defaultCapacity".
 *
 * <p>Following is the reference information for the future optimization on threading: The threads
 * that may access this object concurrently are: * Thread A that makes token request. * Thread B
 * that makes token request. * Thread C that distributes tokens and call the callbacks. (Timer) *
 * Thread D that updates the bw config of a controllable. * Thread E that resets a bucket. * Thread
 * F that unregisters a controllable.
 *
 * <p>The implementation now synchronizes on each context to make sure only one thread is accessing
 * the context object at a time.
 *
...
public class SimpleBWControlService extends TimerTask implements IBWControlService {
          ...
          if (bc == null) {
            ...
            try {
              request.callback.available(
                  context.buckets[request.channel], (long) request.requestToken);
            } catch (Throwable t) {
  ...
  /**
   * Give back the acquired tokens due to failing to accomplish the requested operation or
   * over-charged tokens in the case of best-effort request.
   *
   * @param request
   */
          ...
          if (context.bwConfig != null) {
            if (context.bwConfig[3] >= 0) {
              ...
              if (context.bwConfig[request.channel] >= 0) {
                if (request.type == TokenRequestType.BEST_EFFORT) {
                  context.tokenRc[request.channel] +=
                      requestContext.acquiredToken - request.requestToken;
                } else {
