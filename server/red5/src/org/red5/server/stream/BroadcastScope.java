...
/**
 * Scope type for publishing that deals with pipe connection events, like async message listening in
 * JMS
 */
public class BroadcastScope extends BasicScope implements IBroadcastScope, IPipeConnectionListener {
  ...
  /**
   * Register pipe connection event listener with this scope's pipe. A listener that wants to listen
   * to events when provider/consumer connects to or disconnects from a specific pipe.
   *
        ...
        if (compCounter <= 0) {
          ...
          if (hasParent()) {
            IProviderService providerService =
                (IProviderService) getParent().getContext().getBean(IProviderService.BEAN_NAME);
